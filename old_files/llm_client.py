...
"""
    ...
    def classify_query_type(self, query: str, equipment_context: Dict[str, str]) -> str:
        """
        Classify query as FMECA or MANUAL with robust heuristics + safe LLM fallback.
        Returns either "FMECA" or "MANUAL".
        """
        ...
        fmeca_keywords = {
            "fault", "faults", "failure", "fail", "failed", "not", "doesn't", "doesnt",
            "error", "errors", "alarm", "alarms", "vibration", "leak", "smoke", "tripped",
            "overheat", "overheating", "not starting", "stall", "stalled", "abnormal",
            "symptom", "symptoms", "diagnos", "diagnose", "diagnosis", "root cause",
            "troubleshoot", "troubleshooting", "why"
        }
        manual_keywords = {
            "how to", "how do i", "how does", "how to replace", "procedure",
            "procedure for", "step", "steps", "step-by-step", "install", "installation",
            "remove", "replace", "inspection", "maintenance", "service", "start", "stop",
            "operate", "operation", "operating", "checklist", "safety", "prepare", "assemble",
            "disassemble", "tighten", "loosen", "torque", "calibrate", "adjust", "setting"
        }

        # Quick heuristic scoring
        fmeca_count = sum(1 for kw in fmeca_keywords if kw in ql)
        manual_count = sum(1 for kw in manual_keywords if kw in ql)

        logger.debug(f"Classifier heuristic counts - fmeca: {fmeca_count}, manual: {manual_count} for query: {q[:120]}")

        # Strict heuristic rules (high precision, prefer MANUAL for ambiguous user intents that include how/steps)
        if manual_count >= 1 and fmeca_count == 0:
            logger.debug("Heuristic -> MANUAL (manual keywords present, no fmeca keywords)")
            return "MANUAL"
        if fmeca_count >= 2 and manual_count == 0:
            logger.debug("Heuristic -> FMECA (2+ fmeca keywords, no manual keywords)")
            return "FMECA"

        # If ambiguous but contains explicit "how" or "procedure" tokens prefer MANUAL
        if any(tok in ql for tok in ("how", "how to", "procedure", "step", "steps", "install", "replace")) and fmeca_count <= manual_count:
            logger.debug("Heuristic tie-break -> MANUAL (how/procedure tokens present)")
            return "MANUAL"

        # If the query is very short and contains single keywords (like "vibration"), prefer FMECA
        if len(ql.split()) <= 3 and fmeca_count >= 1 and manual_count == 0:
            logger.debug("Heuristic short-query -> FMECA")
            return "FMECA"
        ...
        try:
            ...
            prompt = (
                ...
                "Your job is to determine whether the user's question is related to 'FMECA' "
                "(Failure Modes, Effects, and Criticality Analysis) or 'MANUAL' "
                "(standard operation, troubleshooting, or procedural reference).\n\n"
                "If you are uncertain, if the query is general, ambiguous, or not explicitly about FMECA analysis, "
                "you must default to 'MANUAL'.\n\n"
                "Return only a JSON object with a single key 'info_type' whose value is either 'FMECA' or 'MANUAL'. "
                "Do NOT include any explanations, comments, or extra text outside the JSON.\n\n"
                f"{few_shot}\n"
                f"Equipment Context: {equipment_info}\n\n"
                f"User Question: {q}\n\n"
                'Respond with ONLY a valid JSON object, for example: {"info_type": "FMECA"}'
            )
            ...
            # Final sanity checks: don't return FMECA if query explicitly asks "how" or "procedure"
            if info_type == "FMECA" and any(k in ql for k in ("how", "how to", "procedure", "step", "steps", "install", "replace")):
                logger.info("LLM predicted FMECA but query contains explicit manual tokens -> overriding to MANUAL")
                return "MANUAL"

            # If model returns unknown value, fallback
            if info_type not in ("FMECA", "MANUAL"):
                logger.warning(f"LLM returned unexpected info_type: {info_type} - falling back to heuristic")
                return "FMECA" if fmeca_count > manual_count else "MANUAL"
