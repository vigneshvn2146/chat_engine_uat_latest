...
class MaritimeRAGApplication:
    ...
    def __init__(self):
        ...
        # 🔥 Create Bedrock client once and reuse it
        try:
            self.bedrock_client = self.create_bedrock_client()
            logger.info("✅ Bedrock client initialized at startup")
        except Exception as e:
    ...
    def _send_ws_chunk(self, connection_id: str, text: str, retries: int = 3,
        ...
        message = {
            ...
            "Prompt": user_prompt,
            "TopicID": topic_id,
            "TopicName": topic_name or "Marine Equipment Query",
    ...
    def stream_llm_response(self, connection_id: str, transaction_id: str,
        ...
        max_chunk_interval: float = 0.05, max_chunk_size: int = 20) -> bool:
        ...
        """
        try:
            bedrock_client = self.bedrock_client or self.create_bedrock_client()
...
If any instruction conflicts with the provided Context or would require inventing facts, follow the Evidence / hallucination guard above and refuse to invent details."""
            ...
            for event in stream_iter:
                ...
                if "contentBlockDelta" in event:
                    ...
                    logger.info("✅ Reached messageStop event, finishing stream")
                    break
            ...
            # --- Resolve topic name before final response ---
            resolved_topic_name = topic_name or "Marine Equipment Query"

            # --- Persist resolved topic name to Dynamo ---
            try:
                timestamp = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")
                if hasattr(dynamo_manager, "table") and dynamo_manager.table:
                    dynamo_manager.table.update_item(
                        ...
                        ExpressionAttributeValues={':t': resolved_topic_name, ':u': timestamp}
                    )
                    logger.info(f"💾 Persisted topic_name='{resolved_topic_name}' for topic_id={topic_id}")
            ...
            # --- Send final response with topic name ---
    ...
    def send_websocket_response(self, connection_id: str, transaction_id: str, 
        ...
        try:
            self.submit_bg(dynamo_manager.add_message, topic_id, dynamo_message)
        except Exception as e:
    ...
    def handle_new_session(self, connection_id: str, transaction_id: str,
        ...
        print(f"📝 TOPIC GENERATION: {topic_name}")
        ...
        try:
            ...
            entity_result = {"response": "False", "error": str(e)}
        ...
        if entity_result.get('response') == "True":
            ...
            # Fire-and-forget persist entity info (no need to block)
            self.submit_bg(dynamo_manager.save_entity_info, topic_id, entity_result)
    ...
    def handle_existing_session(self, connection_id: str, transaction_id: str,
        ...
        if entity_result['response'] == "True":
            ...
            updated_context = {
                'make': entity_result.get('make') or equipment_context['make'],
                'model': entity_result.get('model') or equipment_context['model'],
                'equipment': entity_result.get('equipment') or equipment_context['equipment'],
                'vessel': entity_result.get('vessel') or equipment_context['vessel'],
                'problems': equipment_context['problems'],
                'info_type': entity_result.get('info_type') or equipment_context['info_type'],
                'make_type': entity_result.get('make_type') or equipment_context['make_type'],
                'detail': entity_result.get('detail') or equipment_context['detail'],
                'engine_type': entity_result.get('engine_type') or equipment_context['engine_type']
            }
            ...
            if updates:
                ...
                dynamo_manager.save_entity_info(topic_id, updated_context)
    ...
    def execute_main_processing_flow(self,
            ...
            try:
                ...
                print("🎯 Step 1 & 2: Query Classification + Embeddings (parallel)...")
                ...
                try:
                    ...
                    print(f"⏱️ Classification took {time.time()-t0:.2f}s (cumulative)")
                except concurrent.futures.TimeoutError:
                ...
                equipment_context['info_type'] = info_type or ''

                # ✅ Persist to Dynamo asynchronously
                try:
                    self.submit_bg(dynamo_manager.save_entity_info, topic_id, equipment_context)
                except Exception as e:
                    logger.warning(f"⚠️ Failed to persist updated info_type: {e}")

                # Step 3: Execute Vespa search (offload to thread pool to avoid blocking main thread)
                # print("🔍 Step 3: Executing Vespa search (async)...")
                print("🔍 Step 3: Executing Vespa search (parallel tiers)...")
                tsearch_start = time.time()
                search_results, yql = vespa_client.search_tiers_parallel(
                    query_embedding, rephrased_query, info_type, equipment_context, timeout=12.0
                )
                print(f"⏱️ Vespa parallel search took {time.time()-tsearch_start:.2f}s")

                # search_future = self.executor.submit(
                #     vespa_client.search_with_equipment_context,
                #     query_embedding, rephrased_query, info_type, equipment_context
                # )
                # try:
                #     t1 = time.time()
                #     search_results, yql = search_future.result(timeout=30)
                #     print(f"⏱️ Vespa search took {time.time()-t1:.2f}s")
                # except concurrent.futures.TimeoutError:
                #     logger.warning("⚠️ Vespa search timed out - retrying synchronously as fallback")
                #     search_results, yql = vespa_client.search_with_equipment_context(query_embedding, rephrased_query, info_type, equipment_context)
                # except Exception as e:
                ...
                if getattr(search_results, "empty", False):
                    ...
                    final_topic_id = self.send_websocket_response(
                        connection_id, transaction_id, user_prompt, topic_id,
                        prompt_timestamp, response_data, topic_name = topic_name
                    )
                    lambda_response = {
                        "statusCode": 200,
                        "body": json.dumps({
                ...
                print("📋 Step 5: Preparing context from search results (async)...")
                ...
                streaming_success = self.stream_llm_response(
                    ...
                    yql=yql,
                    topic_name=topic_name,
                    url_mapping={}  # start streaming now; attach real signed urls later
                )
                logger.info(f"⏱️ Streaming took {time.time()-t3:.2f}s")

                # After streaming, if signed urls exist (we scheduled them), try to attach them
                if signed_future:
                    # signed_future is a future created via submit_bg; it returns a Future but was wrapped with a callback.
                    # Recompute signed URLs synchronously with short wait if needed (best-effort)
                    try:
                        # If create_signed_url_mapping returns value, we can fetch it. If submit_bg wraps it, we don't have direct future.
                        # For safety, schedule another submit with short timeout to fetch signed urls
                        signed_map = None
                        # Try to get the signed mapping asynchronously but with a timeout
                        signed_map = self.submit_and_wait(create_signed_url_mapping, url_mapping, timeout=30, fallback=None)
                        if signed_map:
                            logger.info("✅ Signed URL mapping ready — sending URL update to client")
                            update_resp = self.response_formatter.create_standard_response(
                                response_text="Attached signed URLs for referenced documents.",
                                response_type="Update",
                                data_json=[]
                            )
                            # Include the mapping in a small follow-up websocket message
                            self.send_websocket_response(
                                connection_id=connection_id,
                                transaction_id=transaction_id + "-urls",
                                user_prompt=user_prompt,
                                topic_id=topic_id,
                                prompt_timestamp=prompt_timestamp,
                                response_data=update_resp,
                                topic_name=topic_name,
                                url_mapping=signed_map
                            )
                    except Exception as e:
                        logger.warning(f"⚠️ Failed to attach signed URLs: {e}")
    ...
    def lambda_handler(self, event: Dict[str, Any], context: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.is_new_session(topic_id):
                ...
                # non-blocking write
                self.submit_bg(dynamo_manager.add_message, topic_id, user_message, update_summary=False)
                ...
                result = self.handle_new_session(
                    connection_id, transaction_id, user_prompt, topic_id, 
                    prompt_timestamp, is_new_topic = True
                )
