...
class MaritimeRAGApplication:
    ...
    def stream_llm_response(self, connection_id: str, transaction_id: str,
        ...
        max_chunk_interval: float = 0.05, max_chunk_size: int = 3) -> bool:
        ...
        try:
            ...
            ## 🛠️ FMECA Interpretation Rules
            If the Context includes FMECA tables or rows, interpret them using the following fields:
            - **Failure_Mode** → What went wrong
            - **Failure_Causes** → Why it happens
            - **Failure_Effects** → Consequences on equipment
            - **Failure_Symptom** → What the operator observes
            - **Recommended_Checks** → What to inspect or verify
            - **Recommended_Actions** → What to adjust, clean, tighten, replace, or rectify
            - **Severity / Occurrence / Detection** → Use only when relevant to risk or prioritization

            When answering:
            - Use both **manual content** and **FMECA content** together.
            - If FMECA provides a clear failure mode → explain it first.
            - If manual provides step-by-step actions → include them after FMECA reasoning.
            - Keep FMECA terms exactly as written (do not invent new fields).
            - If FMECA contradicts manual, prioritise manual for procedures, FMECA for diagnosis.

            Always cite FMECA entries as:  
            `**FMECA – Failure Mode:** <text>`  
            followed by the source link like normal.
    ...
    def _has_complete_equipment(self, equipment: Dict[str, str]) -> bool:
        ...
        filled = [bool(equipment.get(k)) for k in ['make', 'model', 'equipment']]
        return sum(filled) >= 1
    ...
    def execute_main_processing_flow(self,
            ...
            if not self._has_complete_equipment(equipment_context):
                missing = [k for k in ['make','model','equipment'] if not equipment_context.get(k)]
                logger.info(f"⚠️ Missing {missing}, continuing with available info.")
            ...
            try:
                ...
                search_results, yql = vespa_client.search_with_vespa_retrieval(
