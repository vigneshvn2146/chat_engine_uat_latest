...
class DynamoDBChatManager:
    ...
    def update_summary_with_mistral(self, topic_id):
        ...
        try:
            ...
            # Get LLM summary
            if len(summary_prompt) > 8000:  # or whatever your model limit allows
                logger.warning("⚠️ Summary prompt too long — truncating safely.")
                summary_prompt = summary_prompt[-2000:]  # keep the end (latest context)
            ...
            logger.info(f"✅ Updated chat summary with Claude LLM for session {topic_id}")
            return structured_summary
        except Exception as e:
            logger.error(f"❌ Error updating chat summary with Claude LLM: {str(e)}")
...
"""
    ...
    def _parse_llm_summary(self, llm_summary):
        ...
        try:
            if not llm_summary or not llm_summary.strip().startswith("{"):
                logger.warning("⚠️ LLM summary is empty or not JSON — returning default structure")
                return {
